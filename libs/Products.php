...
class Products
{
    ...
    */
    static function getAttributeValuesByProduct($product, $attr_name, bool $in_stock = true){
        if (is_int($product)){
            $product = static::getProduct($product);
        }

        if ($in_stock){
            // sino hay stock, ... me devuelvo
            if ($product->get_stock_quantity() == 0){
                return; 
            }
        }

        $term_slugs = static::getAttributesFromVariation($product, $attr_name);

        $terms = [];
        foreach ($term_slugs as $ix => $slug){
            $term = get_term_by('slug', $slug, 'pa_'. $attr_name);
    ...
    */
    ...
    static function exportProducts($path = null){
        if (empty($path)){
            $path = __DIR__ . '/../logs/product_export.php';
        }

        $product_ay =static::getAllProducts();

        $prods = [];
        foreach ($product_ay as $product){
            $p_ay = static::dumpProduct($product);
            $prods[] = $p_ay;
        }

        return Files::varExport($path, $prods, null, "if (!class_exists('WC_Product_Attribute')){
    ...
    */
    static function getProductsByTermId($term_id, $in_stock = true){
        global $wpdb;
